...
export class VocabulariesService {
    ...
    isEligibleForCreatingVocab(): Boolean {
        const ifGuestUser = Meteor.user().emails[0].address === this.env.guestUserName && this.vocabCount < this.env.gMaxVocab
        const elseNot = Meteor.user().emails[0].address !== this.env.guestUserName
        return (ifGuestUser || elseNot);
    }
