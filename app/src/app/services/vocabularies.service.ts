...
import {Iclass, IClassWithProperties, Iproperty, Ivocabulary, meteorID, PropertyType} from '../../../api/models';
...
export class VocabulariesService {
  ...
  getClassIDFromVocabForURI(vocabularyId: string, URI: string): Observable<string> {
    return this.getClassIDsForVocabID(vocabularyId).pipe(
        flatMap((classes, _ignored) => {
          const classqueryRes: Observable<Array<{ _id: meteorID }>> = VocabulariesService.wrapFunkyObservables(Classes.find({
            _id: {$in: classes},
            URI
